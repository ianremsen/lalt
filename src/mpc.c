...
int mpc_parse_input(mpc_input_t* i, mpc_parser_t* init, mpc_result_t* final) {
    ...
    while (!mpc_stack_empty(stk)) {
        ...
        case MPC_TYPE_PREDICT:
            if (st == 0) {
                mpc_input_backtrack_disable(i);MPC_CONTINUE(1, p->data.predict.x);
            }
            if (st == 1) {
        ...
        case MPC_TYPE_NOT:
            if (st == 0) {
                mpc_input_mark(i);MPC_CONTINUE(1, p->data.not.x);
            }
            if (st == 1) {
                if (mpc_stack_popr(stk, &r)) {
                    mpc_input_rewind(i);
        ...
        case MPC_TYPE_COUNT:
            if (st == 0) {
                mpc_input_mark(i);MPC_CONTINUE(st + 1, p->data.repeat.x);
            }
            if (st >  0) {
                if (!mpc_stack_peekr(stk, &r)) {
                    mpc_stack_popr(stk, &r);
        ...
        case MPC_TYPE_AND:
            ...
            if (st == 0) {
                mpc_input_mark(i);MPC_CONTINUE(st + 1, p->data.and.xs[st]);
            }
            if (st <= p->data.and.n) {
                if (!mpc_stack_peekr(stk, &r)) {
                    mpc_input_rewind(i);
                ...
                if (st == p->data.and.n) {
                    mpc_input_unmark(i);MPC_SUCCESS(mpc_stack_merger_out(stk, p->data.and.n, p->data.and.f));
                }
            }
...
static int mpc_soi_anchor(char prev, char next) {
    (void) next;return (prev == '\0');
}
static int mpc_eoi_anchor(char prev, char next) {
    (void) prev;return (next == '\0');
}
...
static mpc_val_t* mpcf_re_repeat(int n, mpc_val_t** xs) {
    ...
    if (strcmp(xs[1], "*") == 0) {
        free(xs[1]);return mpc_many(mpcf_strfold, xs[0]);
    }
    if (strcmp(xs[1], "+") == 0) {
        free(xs[1]);return mpc_many1(mpcf_strfold, xs[0]);
    }
    if (strcmp(xs[1], "?") == 0) {
        free(xs[1]);return mpc_maybe_lift(xs[0], mpcf_ctor_str);
    }
...
static mpc_val_t* mpcf_re_escape(mpc_val_t* x) {
    ...
    if (s[0] == '.') {
        free(s);return mpc_any();
    }
    if (s[0] == '^') {
        free(s);return mpc_and(2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str), free);
    }
    if (s[0] == '$') {
        free(s);return mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str), free);
    }
    // Regex Escape
    if (s[0] == '\\') {
...
static void mpc_print_unretained(mpc_parser_t* p, int force) {
    ...
    if (p->type == MPC_TYPE_NOT) {
        mpc_print_unretained(p->data.not.x, 0);printf("!");
    }
    ...
    if (p->type == MPC_TYPE_MANY) {
        mpc_print_unretained(p->data.repeat.x, 0);printf("*");
    }
    ...
    if (p->type == MPC_TYPE_COUNT) {
        mpc_print_unretained(p->data.repeat.x, 0);printf("{%i}", p->data.repeat.n);
    }
...
static mpc_val_t* mpcaf_grammar_repeat(int n, mpc_val_t** xs) {
    ...
    if (strcmp(xs[1], "*") == 0) {
        free(xs[1]);return mpca_many(xs[0]);
    }
    if (strcmp(xs[1], "+") == 0) {
        free(xs[1]);return mpca_many1(xs[0]);
    }
    if (strcmp(xs[1], "?") == 0) {
        free(xs[1]);return mpca_maybe(xs[0]);
    }
    if (strcmp(xs[1], "!") == 0) {
        free(xs[1]);return mpca_not(xs[0]);
    }
