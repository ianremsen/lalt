...
static char mpc_input_getc(mpc_input_t* i) {
    ...
    switch (i->type) {
        ...
        case MPC_INPUT_FILE: c = fgetc(i->file);
...
static int mpc_input_failure(mpc_input_t* i, char c) {
    switch (i->type) {
        ...
        case MPC_INPUT_FILE: fseek(i->file, -1, SEEK_CUR);
            {break;
            }
        case MPC_INPUT_PIPE: {
            if (!i->buffer) {
...
typedef struct {mpc_ctor_t lf;
                void* x;
} mpc_pdata_lift_t;
typedef struct {mpc_parser_t* x;
                char* m;
} mpc_pdata_expect_t;
typedef struct {int (* f)(char, char);
} mpc_pdata_anchor_t;
...
static void mpc_undefine_unretained(mpc_parser_t* p, int force) {
    ...
    switch (p->type) {
        ...
        case MPC_TYPE_APPLY:    mpc_undefine_unretained(p->data.apply.x, 0);
            break;
        case MPC_TYPE_APPLY_TO: mpc_undefine_unretained(p->data.apply_to.x, 0);
            break;
        case MPC_TYPE_PREDICT:  mpc_undefine_unretained(p->data.predict.x, 0);
            break;
        ...
        case MPC_TYPE_OR:  mpc_undefine_or(p);
            break;
        case MPC_TYPE_AND: mpc_undefine_and(p);
            break;
